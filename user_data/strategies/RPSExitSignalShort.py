...
class RPSExitSignalShort(IStrategy):
    """
    RSI PNR Slope Short Strategy - Exit Signal Version
    ...
    Exit: Exit on RSI PNR buy signal or stop loss
    """
    ...
    # Buy space parameters (exit signal for short) - independent optimization
    buy_rsi_lookback = IntParameter(5, 20, default=10, space='buy')
    buy_rsi_percentile_window = IntParameter(100, 200, default=150, space='buy')
    
    # No ROI parameters - this strategy uses only exit signals
    ...
    plot_config = {
        ...
        'subplots': {
            ...
            'RSI PNR Buy (Exit)': {
                ...
                'rsidiffMIN_threshold': {'color': '#66ff66', 'type': 'line'},
            },
            'VWMA Slow Slope': {
            ...
            'Short Signals': {
                ...
                'rsi_pnr_buy_signal': {'color': '#00ff00', 'type': 'scatter'},   # Short exit
            }
        }
    }
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Exit logic: Exit short on RSI PNR buy signal
        """
        # Get RSI (already calculated in populate_entry_trend)
        rsi_source = self.get_rsi_source(dataframe, self.source_type.value)
        dataframe['rsi'] = ta.RSI(rsi_source, timeperiod=14)
        ...
        # Calculate crossover signals for buy
        ...
        # Exit short on RSI PNR buy signal
        dataframe.loc[dataframe['rsi_pnr_buy_signal'], 'exit_short'] = 1
