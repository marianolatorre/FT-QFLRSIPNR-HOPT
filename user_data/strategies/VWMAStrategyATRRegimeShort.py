...
class VWMAStrategyATRRegimeShort(IStrategy):
    """
    VWMA Short Strategy with ATR Volatility Regime Filter
    ...
    atr_ma_period = IntParameter(30, 100, default=50, space='sell')
    atr_multiplier = DecimalParameter(1.0, 3.0, default=1.5, space='sell', decimals=2)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Populate indicators for VWMA short strategy with ATR regime filter
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if regime_column in dataframe.columns:
            # Short entry: crossunder + negative slope (less than max threshold) + high volatility
            short_condition = (
                dataframe['vwma_crossunder'] &
