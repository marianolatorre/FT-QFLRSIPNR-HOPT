...
class VWMAStrategyVolumeRegimeShort(IStrategy):
    """
    VWMA Short Strategy with Volume + Momentum Regime Filter (OBV + RSI)
    ...
    - Only enters when in bearish regime (OBV < OBV_MA AND RSI between 30-50)
    
    Entry conditions:
    - Short: Fast VWMA crosses below Medium VWMA while Slow VWMA has minimum downward slope
    - AND we are in bearish volume/momentum regime
    ...
    obv_ma_period = IntParameter(15, 30, default=20, space='sell')
    rsi_period = IntParameter(10, 20, default=14, space='sell')
    rsi_lower_bear = IntParameter(25, 35, default=30, space='sell')
    rsi_upper_bear = IntParameter(45, 55, default=50, space='sell')
    ...
    plot_config = {
        ...
        'subplots': {
            ...
            'RSI + Regime': {
                ...
                f'bear_regime_{regime_timeframe}': {'color': '#ff0000', 'type': 'scatter'},
            }
        }
    }
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Populate indicators for VWMA short strategy with Volume + Momentum regime filter
        ...
        if self.dp:
            ...
            # Calculate bearish regime
            # Bear regime: OBV < OBV_MA AND RSI between 30-50
            informative['bear_regime'] = (
                (informative['obv'] < informative['obv_ma']) & 
                (informative['rsi'] >= self.rsi_lower_bear.value) &
                (informative['rsi'] <= self.rsi_upper_bear.value)
            ).astype(int)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        regime_column = f'bear_regime_{self.regime_timeframe}'
        if regime_column in dataframe.columns:
            # Short entry: crossunder + negative slope (less than max threshold) + bearish regime
            short_condition = (
                dataframe['vwma_crossunder'] &
                (dataframe['vwma_slow_slope_clean'] < self.max_slope_slow.value) &
                (dataframe[regime_column] == 1) &  # Only enter in bearish volume/momentum regime
                (dataframe['volume'] > 0)
            )
        else:
