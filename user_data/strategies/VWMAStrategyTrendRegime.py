...
class VWMAStrategyTrendRegime(IStrategy):
    """
    VWMA Strategy with Trend Regime Filter (EMA + ADX)
    ...
    - Only enters when in trending regime (EMA_fast > EMA_slow AND ADX > threshold)
    
    Entry conditions:
    - Long: Fast VWMA crosses above Medium VWMA while Slow VWMA has minimum upward slope
    - AND we are in trending regime
    ...
    ema_fast_period = IntParameter(30, 50, default=40, space='buy')
    ema_slow_period = IntParameter(60, 100, default=80, space='buy')
    ...
    plot_config = {
        ...
        'subplots': {
            ...
            'Trend Regime': {
                ...
                f'trend_regime_{regime_timeframe}': {'color': '#ff00ff', 'type': 'scatter'},
            }
        }
    }
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Populate indicators for VWMA strategy with Trend regime filter
        ...
        if self.dp:
            ...
            # For long strategy: trending up when EMA_fast > EMA_slow AND ADX > threshold
            informative['trend_regime'] = (
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        AND we are in trending regime (uptrend with strong ADX)
        ...
        regime_column = f'trend_regime_{self.regime_timeframe}'
        if regime_column in dataframe.columns:
            long_condition = (
                dataframe['vwma_crossover'] &
                (dataframe['vwma_slow_slope_clean'] > self.min_slope_slow.value) &
                (dataframe[regime_column] == 1) &  # Only enter in trending regime
                (dataframe['volume'] > 0)
            )
        else:
